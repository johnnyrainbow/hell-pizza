...
class Menu {
	...
	rangeRequest(store_id, range, callback) {
		if (!store_id) return callback(status.error.no_provided_store_id);

		var formatted_url = util.formatStoreURL(url.menu.full_menu, {
			store_id: store_id,
		});
		httpJson.get(formatted_url, function (err, response) {
			if (err) return callback(err);

			const items = response.payload.menu.items;
			const result = [];
			// console.log(items);
			...
			return callback(null, {
				items: result,
				store_info: response.payload.store,
			});
		});
	}
}
